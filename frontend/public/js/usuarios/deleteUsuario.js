//script para la ejecucion del modal de manera dinamica

document.addEventListener("click", async (event) => {
    let target = event.target;
  
    //si el click fue en la img de trash, que tome el objetivo del btn padre
    if(target.tagName === "IMG" && target.closest(".btn-trash")){
      target = target.closest(".btn-trash").querySelector("button");
   }

    // Verifica si se hizo clic en el .btn-trash
    if(target && target.matches(".btn-trash button") || target.matches(".btn-trash img")){
      const idUsuario = target.dataset.idUsuario; // Obtiene el ID de la especialidad desde el atributo data

      console.log("ID usuario: ", idUsuario);
      
      // hay problema al encontrar los usuarios
      if(!idUsuario){//si no se obtuvo la idEspecialidad lanza un error en el nav console
        console.error("no se encontro el id del usuario de este registro");
        return;
      }

      try {
        // Petición para obtener los datos de la especialidad
        const response = await fetch(`http://localhost:3000/usuarios/${idUsuario}`);
        if (!response.ok) throw new Error("Error al obtener los datos");//linea optimizada
        const data = await response.json();// datos recibe toda la info del fetch
        const usuario = data[0];//se accede al primer objeto del array
        // Llamamos a la función para actualizar el modal
        actualizarModal(usuario);
      } catch (error) {
        console.error("Hubo un problema:", error);
      }
    }
  });
  
  // Función para actualizar el contenido del modal
  function actualizarModal(usuario) {

    if (!usuario) {
      console.error("No se encontraron datos válidos para el usuario");
      return;
    }
    //ocurria un error porque parece ser que el fetch devuelve un array con un 
    // objeto dentro en lugar de un objeto directamente. Lo vemos porque console.log(data) 
    // imprime [{...}] (un array con un solo objeto). ver en linea 26:27

    // Referencias a los elementos del modal
    const modal = document.querySelector(".modal");
    if (!modal){
      console.error("no se encontro el modal en el DOM");
      return;
    }

    const idField = modal.querySelector("#idUsuario span");
    const nombreField = modal.querySelector("#nombreUsuario span");
    const emailField = modal.querySelector("#correoElectronico span");
    const contraseñaField = modal.querySelector("#password span");
    const fechaNacimientoField = modal.querySelector("#fechaNacimiento span");
    const idRolField = modal.querySelector("#idRol span");
    const idGeneroField = modal.querySelector("#idGenero span");
    const fechaField = modal.querySelector("#fechaAltaUsuario span");
    const imgField = modal.querySelector(".previa-img img");
    const avisoField = modal.querySelector(".texto-aviso span");
    const btnEliminar = modal.querySelector(".btn-eliminar");

    if(!idField || !nombreField || !emailField || !contraseñaField  || !fechaNacimientoField  || !idGeneroField  || !idRolField || !fechaField || !imgField || !btnEliminar){
      console.error("uno o mas elementos del modal no fueron encontrados");
      return;
    }

    console.log(usuario);//depuracion

    const baseUrl = "http://localhost:3000/"; 
    const imgPath = usuario.imagen_perfil_usuario.replace(/^\.?\//, ""); // Elimina "./" si existe
    const imgURL = baseUrl + imgPath;

  
    // Asignamos los valores dinámicos
    idField.textContent = usuario.id_usuario || "No disponible";
    nombreField.textContent = usuario.nombre_usuario  || "No disponible";
    emailField.textContent = usuario.correo_electronico  || "No disponible";
    contraseñaField.textContent = usuario.password || "No disponible";
    fechaNacimientoField.textContent = usuario.fecha_nacimiento || "No disponible";
    idRolField.textContent = usuario.id_rol || "No disponible";
    idGeneroField.textContent = usuario.id_genero || "No disponible";
    fechaField.textContent = usuario.fecha_alta_sistema  || "No disponible";
    imgField.src = imgURL; // si no hay imagen, dejamos vacio
    imgField.alt = usuario.imagen_perfil_usuario || "Imagen no disponible";

    //
    avisoField.textContent = usuario.id_usuario || "undefinded";

    // configuramos el boton de eliminacion con el id
    btnEliminar.dataset.id = usuario.id_usuario;

    console.log("Datos cargados en el modal:", {
      ID: idField.textContent,
      Nombre: nombreField.textContent,
      email: emailField.textContent,
      contraseña: contraseñaField,
      FechaNacimiento: fechaNacimientoField,
      IdRol: idRolField,
      IdGenero: idGeneroField,
      FechaActualizacion: fechaField.textContent,
      Imagen: imgField.src,
    });
  
    // Mostramos el modal
    modal.classList.add("modal--show");// muestra el modal
    
  }
  
  // Evento para cerrar el modal al hacer clic en "Cancelar"
  document.querySelector(".btn-cancelar").addEventListener("click", () => {
   const modal = document.querySelector(".modal");
    modal.classList.remove("modal--show");//ocultamos el modal
  });
  
  // Evento para eliminar la especialidad
  document.querySelector(".btn-eliminar").addEventListener("click", async (event) => {
    const idUsuario = event.target.dataset.id;
  
    try {
      const response = await fetch(`http://localhost:3000/usuarios/${idUsuario}`, { method: "DELETE" });
      if (!response.ok) throw new Error("Error al eliminar el registro");
      alert("Registro eliminado con éxito");
      document.querySelector(".modal").classList.remove("modal--show"); //elimina el modal, antes lo ocultaba
      //actuallizar o renderizar autoaticamente la tabla
      ObtenerUsuarios();;
    } catch (error) {
      console.error("Hubo un problema al eliminar:", error);
      alert("Hubo un problema al eliminar:", error)
    }
  });